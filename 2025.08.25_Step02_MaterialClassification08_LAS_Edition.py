...
def save_colored_pointcloud(out_path_base: str,
    ...
    """
    Save colored point cloud to both PLY and LAS.
    Points: (N,3), Colors: (N,3) uint8
    """
    # --- Save as PLY (broadly supported) ---
    pcd = o3d.geometry.PointCloud()
    ...
    # --- Save as LAS (for point cloud viewers) ---
    try:
        import laspy
...
def main():
    ...
    if room is not None:
        colors = assign_colors_from_box(points, room, dist_wall=0.25, dist_floor=0.05, dist_ceil=0.05, z_margin=0.10)
        colors = assign_colors_from_box(points, room, door_window_override=True, df=df, by_iid=by_iid)
        save_plotly_html(out_base, room, points, colors, annotations)
        save_colored_pointcloud(out_base, points, colors)
