...
def assign_colors_from_box(points: np.ndarray,
                           room: RoomFit,
                           dist_thresh: float = 0.15) -> np.ndarray:
    """
    Color each point by the nearest snapped box face.
    Uses COLOR_TABLE hex strings -> RGB.
    """
    default_gray = hex2rgb(COLOR_TABLE["Other"])

    faces = [
        ("Wall East",  snapped_wall_corners("east",  room)),
        ("Wall North", snapped_wall_corners("north", room)),
        ("Wall West",  snapped_wall_corners("west",  room)),
        ("Wall South", snapped_wall_corners("south", room)),
        ("Floor",      room.floor_corners),
        ("Ceiling",    room.ceiling_corners),
    ]
    ...
    dists = np.abs(points @ normals.T + offsets[None, :])
    nearest_idx = np.argmin(dists, axis=1)
    nearest_dist = dists[np.arange(dists.shape[0]), nearest_idx]

    colors = np.tile(default_gray[None, :], (points.shape[0], 1))
    face_names = [name for name, _ in faces]

    for i, name in enumerate(face_names):
        mask = (nearest_idx == i) & (nearest_dist < dist_thresh)
        if np.any(mask):
            colors[mask] = hex2rgb(COLOR_TABLE[name])
...
def main():
    ...
    if room is not None:
        colors = assign_colors_from_box(points, room)
        save_plotly_html(out_base, room, points, colors, annotations)
