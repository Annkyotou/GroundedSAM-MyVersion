# Step01_scene_classification.py
...
def save_scene_type(label, confidence, out_path):
    with open(out_path, "w") as f:
        f.write(f"{label}, confidence: {confidence:.3%} \n")
    print(f"[✓] Scene type saved to {out_path}")
...
def main():
    ...
    scene_txt_path = f"./outputs/{base_name}_scene_type.txt"
    npz_path = f"./outputs/{base_name}_used_points.npz"
    ...
    np.savez(npz_path, used_points=used_points, sensor_pos=sensor_pos, colors=used_colors)
    print(f"[{timestamp()}] [✓] Saved used points for label projection: {npz_path}")
    ...
    scene_label, confidence = classify_scene(pano_image, model, preprocess, tokenizer, device)
    save_scene_type(scene_label, confidence, scene_txt_path)
